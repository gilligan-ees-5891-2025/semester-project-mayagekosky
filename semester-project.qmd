...
data_vals_fgg <- as.numeric(df$`...2`[c(11, 14, 15, 16)])  # use your column & rows

qqnorm(data_vals_fgg)
qqline(data_vals_fgg, col = "purple4", lwd = 2)

#making a q-q plot for intermediate samples
data_vals_fgg <- as.numeric(df$`...2`[c(8, 10, 13, 17)])  # use your column & rows

qqnorm(data_vals_fgg)
qqline(data_vals_fgg, col = "magenta4", lwd = 2)
```
...
Al2O3_Plot <- ggplot(df_combined_Al, aes(x = SiO2, y = Al2O3, color = units)) +
  ...
  geom_segment(
   ...
   geom_point(data = df_combined |> filter(units != "fgg (avg)"), size = 3) +
 geom_smooth(
  data = df_combined_Al |>
